...
function(out,xlu=NaN,no.col=FALSE,standard=TRUE)
{
  ...
  if(no.col==FALSE)
  {
    ...
    if(is.null(out))
    {
      return("Run 'JOP' first! 'locate' needs output of 'JOP'!")
    }
    if(!is.nan(xlu))
    {
      if(xlu<1 || xlu>numbW)
    ...
    legend("topright",dimnames(out$Parameters)[[2]],col=cols[1:nx],bty="n",lwd=1)
  ...
  else
  {
    ...
    legend("topright",dimnames(out$Parameters)[[2]],lty=1:nx,col=cols3,bty="n",lwd=1)
